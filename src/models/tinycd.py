...
from torch.nn import Conv2d, InstanceNorm2d, Module, ModuleList, PReLU, Sequential, Upsample, Identity
...
class UpMask(Module):
    def __init__(
        ...
        self._convolution = Sequential(
            Conv2d(nin, nin, kernel_size=3, stride=1, groups=nin, padding=1),
            PReLU(),
            InstanceNorm2d(nin),
...
class TinyCD(Module):
    def __init__(
        ...
        self._backbone[0][0] = Conv2d(
            in_channels=input_nc,
            out_channels=self._backbone[0][0].out_channels,
            kernel_size=self._backbone[0][0].kernel_size,
            stride=self._backbone[0][0].stride,
            padding=self._backbone[0][0].padding,
            bias=self._backbone[0][0].bias is not None)

        

        # Initialize mixing blocks:
        self._first_mix = MixingMaskAttentionBlock(2*input_nc, 3, [3, 10, 5], [10, 5, 1])
    ...
    def forward(self, ref: Tensor, test: Tensor, dem: Tensor = None) -> Tensor:
        # NB: dem is not used
