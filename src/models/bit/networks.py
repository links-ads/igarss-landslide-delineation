...
def define_G(arch, input_nc, output_nc=2, init_type='normal', init_gain=0.02):
    if arch == 'base_resnet18':
        net = ResNet(input_nc=3, output_nc=output_nc, output_sigmoid=False)

    elif arch == 'base_transformer_pos_s4':
        net = BASE_Transformer(input_nc=input_nc, output_nc=output_nc, token_len=4, resnet_stages_num=4,
                             with_pos='learned')
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            self.encoder = resnet18(pretrained=True, replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet34':
            self.encoder = resnet34(pretrained=True, replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet50':
            self.encoder = resnet50(pretrained=True, replace_stride_with_dilation=[False,True,True])
            expand = 4
        else:
    ...
    def forward_single(self, x):
        ...
        x_4 = self.encoder.layer1(x) # 1/4, in=64, out=64
        x_8 = self.encoder.layer2(x_4) # 1/8, in=64, out=128

        if self.resnet_stages_num > 3:
            x_8 = self.encoder.layer3(x_8) # 1/8, in=128, out=256

        if self.resnet_stages_num == 5:
            x_8 = self.encoder.layer4(x_8) # 1/32, in=256, out=512
